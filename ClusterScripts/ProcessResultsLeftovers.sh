...
while (($experiment <= $TotalExperiments)); do
	...
	if [ ! -f Everything.mat ]; then
		echo "$CURRENTDIR"
		qsub -l h_rt=02:00:00 -l tmem=8.0G -l h_vmem=8.0G -l s_stack=10240 -j y -b y -cwd -V matlab -nodesktop -nodisplay -nosplash -nojvm -r letsprocesscluster
	fi
	for ((j=1;j<=NumVariables;j++)) do
		cd ..
	done
