function PlotClusteredVsDistributed( AllNumbers, Load_Directories, Save_Directories, Save_Directory, savepdf )

%%%%%%%%%
%Prefer 3rd one probably??
%%%%%%%%
FontSizeAll=18;
for CurrentExperiment = [1:prod(AllNumbers)]
	...
	SimulationFinished = dlmread([Load_CurrentDirectory 'Finished.dat']);
	...
	load([Load_CurrentDirectory 'Everything.mat'], 'VideoConcentrations', 'CellColumns', 'NumColumns', 'DijkstraAll');
	...
	if(savepdf)
		set(gcf, 'Color', 'w');
		export_fig([Save_CurrentDirectory 'Distributedness.pdf'], '-pdf', '-painters', '-a1', '-q101')
	end
